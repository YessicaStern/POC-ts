...
import { Service,User } from "../protocols/protocols.js";
...
const deleteServicesRepository = (id:number):Promise<QueryResult<string>>=>{
...
const getIdMeetingRepository= (userId:number, date:string, hour:string): Promise <QueryResult<string>>=>{
    return connection.query(`SELECT * FROM meetings WHERE "userId"=$1 AND date=$2 AND hour=$3 `,
    [userId,date,hour]);
};
const getServiceId = (id:number):Promise<QueryResult<string>>=>{
